...
class Calculator extends Component {
    ...
    showOrders(orders) {
        if(!orders || orders.length == 0){
            return <li className = "text-rignt text-muted title">ไม่มีสินค้าค่ะ</li>
        } else {
    ...
    render() {
        ...
        return (
            <div>
                <h1 className = "text-ringht">{totalPrice}</h1>
                <hr/>
                <ul className= "list-unstyled">
                    {this.showOrders(orders)}
                </ul>
                <hr/>
                <button className = "btn btn-block btn-danger title"onClick = {() => this.props.confirmOrder()}>ยืนยัน</button>
